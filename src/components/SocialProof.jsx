    ...
    <div className="flex items-center justify-between mb-3">
      <div className="flex gap-0.5">
        {[...Array(5)].map((_, i) => (
          <Star
            key={i}
            className={`w-4 h-4 ${i < review.rating ? 'fill-yellow-400 text-yellow-400' : 'fill-neutral-200 text-neutral-200'}`}
          />
        ))}
      </div>
      {review.time && <span className="text-xs text-neutral-400">{review.time}</span>}
    </div>
    <p className="text-neutral-600 font-light italic leading-relaxed mb-4">"{review.text}"</p>
    <div className="flex items-center gap-3">
      {review.profilePhoto ? (
        <img src={review.profilePhoto} alt={review.author} className="w-8 h-8 rounded-full object-cover" loading="lazy" />
      ) : (
...
const SkeletonCard = () => (
  <div className="bg-white p-6 border border-neutral-200 animate-pulse">
    <div className="flex gap-1 mb-3">
      {[...Array(5)].map((_, i) => <div key={i} className="w-4 h-4 bg-neutral-200 rounded" />)}
    </div>
...
export default function SocialProof() {
    ...
    const fetchReviews = async () => {
      try {
        ...
        if (data.reviews?.length > 0) {
          ...
          setReviewData({ reviews: FALLBACK_REVIEWS, rating: 5.0, totalReviews: FALLBACK_REVIEWS.length, configured: data.configured || false })
        }
      } catch {
        setReviewData({ reviews: FALLBACK_REVIEWS, rating: 5.0, totalReviews: FALLBACK_REVIEWS.length, configured: false })
      } finally {
  ...
  const skeletonsToShow = loading ? SKELETON_COUNT : Math.max(0, SKELETON_COUNT - reviews.length)

  return (
    <section className="py-32 bg-neutral-50">
      <div className="container-custom">
        <div ref={sectionObserver.ref} className={`text-center mb-16 animate-fade-up ${sectionObserver.isVisible ? 'visible' : ''}`}>
          <h2 className="text-4xl md:text-5xl font-display font-light text-neutral-900 mb-4">
            What Our <span className="text-accent-blue">Customers Say</span>
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-6">
            <div className="flex flex-col items-center gap-2">
              ...
              <div className="flex items-center gap-2">
                ...
                {reviewData && <span className="text-sm text-neutral-500">{reviewData.rating} ({reviewData.totalReviews} reviews)</span>}
              </div>
            </div>
            <a href={GOOGLE_BUSINESS_URL} target="_blank" rel="noopener noreferrer" className="inline-flex items-center gap-2 px-6 py-3 bg-neutral-900 text-white font-medium text-sm tracking-wider uppercase hover:bg-accent-blue transition-all duration-300">
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          {Array.from({ length: skeletonsToShow }).map((_, index) => <SkeletonCard key={`skeleton-${index}`} />)}
        </div>
        {skeletonsToShow > 0 && (
        ...
        <div className="flex justify-center mt-12">
          <BookingWidget id="social-proof-booking-widget" />
        </div>
      </div>
    </section>
  )
